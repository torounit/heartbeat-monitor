...
const existingLocations = await db.query.devices.findMany();
...
if (existingLocations.length === 0) {
  ...
  if (insertedLocations.length > 0) {
    const deviceId = insertedLocations[0].id;
    await db.insert(schema.heartbeats).values([
      { deviceId, createdAt: "2025-01-01 00:00:00" },
      { deviceId, createdAt: "2025-01-02 00:00:00" },
      { deviceId, createdAt: "2025-01-03 00:00:00" },
    ]);
  }
}
